(function () {
  ...
  if (!document.getElementById('spac-style')) {
    ...
    st.textContent = [
      ...
      '.spac-search{width:100%;font-size:16px;font-family:"Noto Sans KR",sans-serif;padding:14px 16px 14px 42px;border:1.5px solid var(--spac-line);border-radius:12px;background:var(--spac-card);color:var(--spac-ink);outline:none;transition:border-color .15s}',
      '.spac-search:focus{border-color:var(--spac-accent)}',
      '.spac-search-ic{position:absolute;left:15px;top:50%;transform:translateY(-50%);font-size:16px;opacity:.5}',
  ...
  function deadCard(x, cls, label) {
    return '<div class="spac-card2"><div class="spac-simple"><div class="spac-info">' +
      ...
      '<div class="spac-meta">' + esc(x.broker) + '</div>' +
      '</div><span class="spac-pill ' + cls + '">' + label + '</span></div></div>';
  }
