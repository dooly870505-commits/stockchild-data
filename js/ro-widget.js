(function () {
  ...
  /* 카드 클릭 즉시: 이미 갖고 있는 목록용 요약 정보로 뼈대(제목, 배지, 로딩중 표시)만 먼저 그림 */
  ...
  /* 상세 데이터(ev, 시세/코호트 포함)를 실제로 받은 뒤 전체 화면을 그림 */
  ...
  /* 카드 클릭 진입점: 뼈대 먼저 보여주고, 상세 데이터는 별도로 요청 */
  function openDetail(evSummary) {
    renderDetailSkeleton(evSummary);

    fetch(WEBAPP_URL + '?tool=rightsoffering&eventId=' + encodeURIComponent(evSummary.eventId))
      .then(function (res) { return res.json(); })
      .then(function (data) {
        if (data.error) {
          viewDetail.innerHTML = '<div class="ro-back" id="ro-back-btn">← 목록으로</div><div class="ro-empty">상세 정보를 찾지 못했어요</div>';
          document.getElementById('ro-back-btn').addEventListener('click', function () {
            viewDetail.style.display = 'none';
            viewList.style.display = 'block';
          });
        ...
        renderDetailFull(data);
      })
      .catch(function (err) {
        viewDetail.innerHTML = '<div class="ro-back" id="ro-back-btn">← 목록으로</div><div class="ro-empty">상세 정보를 불러오지 못했어요</div>';
        document.getElementById('ro-back-btn').addEventListener('click', function () {
          viewDetail.style.display = 'none';
          viewList.style.display = 'block';
        });
